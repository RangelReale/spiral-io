...
#include "spiral/log/windows/SinkOptions.h"
#include "spiral/log/SinkOptions.h"
#include "spiral/log/Logger.h"
#include "spiral/log/OstreamOutput.h"
#include "spiral/memory.h"
...
static int eventTypes[spiral::log::LevelTraits::COUNT] = {
...
class EventLogOutput : public spiral::log::Logger::Output {
public:
...
spiral::log::SinkOptions& SinkOptions::operator=(const spiral::log::SinkOptions& rhs) {
...
void SinkOptions::setup(spiral::log::Logger *logger) {
    if (logToStderr)
        logger->output(make_auto_ptr<spiral::log::Logger::Output>
                         (new spiral::log::OstreamOutput(std::clog)));
    if (logToStdout)
        logger->output(make_auto_ptr<spiral::log::Logger::Output>
                         (new spiral::log::OstreamOutput(std::cout)));

    if (logFile.length() > 0)
        logger->output(make_auto_ptr<spiral::log::Logger::Output>
                         (new spiral::log::OstreamOutput(logFile)));
...
SinkOptions* SinkOptions::create(const std::string& argv0) {
    return new spiral::log::windows::SinkOptions (argv0);
}
