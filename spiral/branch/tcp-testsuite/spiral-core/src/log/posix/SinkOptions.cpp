...
#include "twisted/log/posix/SinkOptions.h"
#include "twisted/log/SinkOptions.h"
#include "twisted/log/Logger.h"
#include "twisted/log/OstreamOutput.h"
#include "twisted/memory.h"
#include "twisted/Exception.h"
#include <iostream>
#include <map>
#include <string>
#include <syslog.h>
...
static int priorities[twisted::log::LevelTraits::COUNT] = {
...
class SyslogOutput : public twisted::log::Logger::Output {
public:
...
twisted::log::SinkOptions& SinkOptions::operator=(const 
...
void SinkOptions::setup(twisted::log::Logger *logger) {
    if (logToStderr)
        logger->output(make_auto_ptr<twisted::log::Logger::Output>
                         (new twisted::log::OstreamOutput(std::clog)));
    if (logToStdout)
        logger->output(make_auto_ptr<twisted::log::Logger::Output>
                         (new twisted::log::OstreamOutput(std::cout)));

    if (logFile.length() > 0)
        logger->output(make_auto_ptr<twisted::log::Logger::Output>
                         (new twisted::log::OstreamOutput(logFile)));
...
SinkOptions* SinkOptions::create(const std::string& argv0) {
    return new twisted::log::posix::SinkOptions (argv0);
}

}} // namespace twisted::log
